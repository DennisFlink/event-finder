...
const getEventsByFilter = async (filterData) => {
	const user = await User.find({
		username: { $regex: filterData.author, $options: 'i' },
	});
	const userIds = user ? user.map((user) => user._id.toString()) : null;

	const query = {
		...(filterData.title && {
			title: { $regex: filterData.title, $options: 'i' },
		}),
		...(filterData.location && {
			location: { $regex: filterData.location, $options: 'i' },
		}),
		...(filterData.ageLimit && {
			ageLimit: { $gte: Number(filterData.ageLimit) },
		}),
		...(filterData.needsApproval !== undefined && {
			needApproval: filterData.needsApproval === 'true',
		}),
		...(filterData.date?.from &&
			!filterData.date?.to && {
				startDate: {
					...
					$lte: new Date(
						new Date(filterData.date.from).setHours(23, 59, 59, 999),
					),
				},
			}),
		...
		...(filterData.date?.from &&
			filterData.date?.to && {
				startDate: {
					$gte: new Date(new Date(filterData.date.from).setHours(0, 0, 0, 0)),
					$lte: new Date(
						new Date(filterData.date.to).setHours(23, 59, 59, 999),
					),
				},
			}),
		isPaymentRequired: filterData.isPaymentRequired === 'true',
		...(filterData.totalAttendees && {
			totalAttendees: { $gte: filterData.totalAttendees },
		}),
		...(userIds.length > 0 && { authorId: { $in: userIds } }),
	};

	const events = await Events.find(query);
	console.log('Evenfdsts', events);
	return events;
};
